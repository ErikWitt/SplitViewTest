...
- (void)viewDidLoad
{
    ...
    //TODO: find better ButtonStyle than UIBarButtonSystemItemDone
    UIBarButtonItem* homeButton = [[UIBarButtonItem alloc] initWithBarButtonSystemItem: UIBarButtonSystemItemDone target:self action:@selector(homeButtonClicked)];
...
-(void) collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [detailNavigationController pushViewController:viewController animated:NO];
}

-(void) homeButtonClicked
{
    UINavigationController* masterNavigationController = [self.splitViewController.viewControllers objectAtIndex:0];
    [masterNavigationController popToRootViewControllerAnimated:YES];
}
@end
